...
def load_and_prepare_data(
    ...
    # Load training data
    X_train = pd.read_excel(train_path, sheet_name="x")
    y_train = pd.read_excel(train_path, sheet_name="y")
    X_train["y"] = y_train

    # Load testing data
    X_test = pd.read_excel(test_path, sheet_name="x")
    y_test = pd.read_excel(test_path, sheet_name="y")
    X_test["y"] = y_test
...
def get_txt_file(dataframe, cols, filename):
    # Base directory: Code/
    BASE_DIR = Path(__file__).resolve().parent.parent

    # Output directory
    OUTPUT_DIR = BASE_DIR / "Datasets" / "processed" / "Datasets csvs"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    # Select columns
    X = dataframe[cols].reset_index(drop=True)
    X = X.rename_axis("Experiment")

    # Save file
    output_path = OUTPUT_DIR / filename
    X.to_csv(output_path, sep="\t")

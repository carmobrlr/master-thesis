...
def distribution_discretizer(dataset, number_of_bins, statistical_test="chi2", cutoff_margin=0.2, kolmogorov_opt=False,
    ...
    if distributions is None:
        distributions = ["alpha", "anglit", "arcsine", "argus", "beta", "betaprime", "bradford",
                         ...
                         "gibrat", "gumbel_r", "gumbel_l", "halfcauchy", "halflogistic",
...
def discritizer_CM_content(dataset, col):
    for i in range(len(dataset)):
        if (dataset.loc[i, col] <= 50):
            dataset.loc[i, col] = 0 # low CM content
        elif (dataset.loc[i, col] < 80 and dataset.loc[i,col]>50):
            dataset.loc[i, col] = 1 # medium CM content
        else:
            dataset.loc[i, col] = 2 # high CM content
    return dataset
